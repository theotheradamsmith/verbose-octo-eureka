...
func parseGrid(grid string) (map[string]string, error) {
	// Convert grid to a map of possible values, {square: digits}, or return an error
...
func Logic() {
	...
	/*
		if gv, ok := GridValues("4.....8.5.3..........7......2.....6.....8.4......1.......6.3.7.5..2.....1.4......"); ok != nil {
			fmt.Println(ok)
		} else {
			fmt.Println(gv)
			var ret []string
		...
		if gv, ok := GridValues("abc"); ok != nil {
			fmt.Println(ok)
		} else {
			fmt.Println(gv)
		}
	*/
	//fmt.Println(Verify(gv))
	return
}
